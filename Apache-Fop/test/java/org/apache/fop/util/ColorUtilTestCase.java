...
public class ColorUtilTestCase {
    ...
    public void testEquals() throws Exception {
        ...
        col1 = ColorUtil.parseColorString(null,
                "fop-rgb-icc(0.5,0.5,0.5,#CMYK,,0.0,0.0,0.0,0.5)");
        /*
         * The following doesn't work since java.awt.Color from Sun doesn't
         * round consistently col2 = ColorUtil.parseColorString(null,
         * "cmyk(0.0,0.0,0.0,0.5)"); assertEquals(col1, col2);
         */

        col2 = ColorUtil.parseColorString(null,
                "fop-rgb-icc(0.5,0.5,0.5,#CMYK,,0.5,0.5,0.5,0.0)");
        assertTrue(col1.equals(col2));
        assertFalse(org.apache.xmlgraphics.java2d.color.ColorUtil.isSameColor(
                col1, col2));
    }
    ...
    public void testRGB() throws Exception {
        ...
        assertEquals(ColorSpace.getInstance(ColorSpace.CS_sRGB),
    ...
    public void testRGBICC() throws Exception {
        final FopFactory fopFactory = FopFactory.newInstance();
        final URI sRGBLoc = new URI(
                "file:src/java/org/apache/fop/pdf/sRGB%20Color%20Space%20Profile.icm");
        final ColorSpace cs = fopFactory.getColorSpaceCache().get("sRGBAlt",
                null, sRGBLoc.toASCIIString(), RenderingIntent.AUTO);
        ...
        assertEquals(0, colActual.getBlue());
        // I don't understand the difference. Maybe Java's sRGB and HP's sRGB
        // are somehow not
        // equivalent. This is only going to be a problem if anyone actually
        // makes use of the
        // RGB fallback in any renderer.
        // TODO Anyone know what's going on here?
        final float[] comps = colActual.getColorComponents(null);
        ...
        final Color fallback = colActual.getFallbackColor();
        ...
        final Color colFallback = ColorUtil.parseColorString(ua, colSpec);
    ...
    public void testCMYK() throws Exception {
        ...
        colActual = (ColorWithAlternatives) ColorUtil.parseColorString(null,
                colSpec);
        assertEquals(255, colActual.getRed());
        ...
        assertEquals(0f, comps[3], 0);
        assertEquals(
                "fop-rgb-icc(0.9726,0.7804,0.67840004,#CMYK,,0.0274,0.2196,0.3216,0.0)",
                ColorUtil.colorToString(colActual));
        ...
        // Verify that the cmyk() and fop-rgb-icc(#CMYK) functions have the same
        // results
        final ColorWithAlternatives colCMYK = (ColorWithAlternatives) ColorUtil
                .parseColorString(null, "cmyk(0,0,0,0.5)");
        assertEquals(colCMYK.getAlternativeColors()[0],
        ...
        // Color(ColorSpace cspace, float components[], float alpha): 0.5 -->
        // 127
        // Color(float r, float g, float b): 0.5 --> 128
        if (!colCMYK.equals(colActual)) {
            log.info("Info: java.awt.Color does not consistently calculate"
    ...
    public void testSeparationColor() throws Exception {
        ...
        colActual = (ColorWithFallback) ColorUtil.parseColorString(null,
                colSpec);
    ...
    public void testNamedColorProfile() throws Exception {
        final FopFactory fopFactory = FopFactory.newInstance();
        final URI ncpLoc = new URI("file:test/resources/color/ncp-example.icc");
        final ColorSpace cs = fopFactory.getColorSpaceCache().get("NCP", null,
                ncpLoc.toASCIIString(), RenderingIntent.AUTO);
        assertNotNull("Color profile not found", cs);

        final FOUserAgent ua = fopFactory.newFOUserAgent();
        ColorWithFallback colActual;

        // fop-rgb-named-color() is used instead of rgb-named-color() inside
        // FOP!
        final String colSpec = "fop-rgb-named-color(1.0,0.8,0.0,NCP," + "\""
