...
class NestedTable extends TestDocument {
    ...
    protected void generateDocument(final RtfDocumentArea rda,
        ...
        sect.newParagraph().newText(
                "This document demonstrates pseudo-nested "
    ...
    private void firstTestTable(final RtfSection sect) throws IOException {

        sect.newParagraph().newText(
                "First test: table with one nested table in cell 1,1");
        final RtfTable tbl = sect.newTable(new DummyTableColumnsInfo());
        // first row, normal
        {
            final RtfTableRow r = tbl.newTableRow();
            final RtfTableCell c = r.newTableCell(160 * MM_TO_TWIPS);
            c.newParagraph().newText(
                    "cell 0,0, width 160mm, only cell in this row.");
        }

        // second row contains nested table
        {
            final RtfTableRow r = tbl.newTableRow();
            r.newTableCell(40 * MM_TO_TWIPS)
                    .newParagraph()
                    .newText(
            ...
            c.newParagraph()
                    .newText(
                            "cell 1,1, width 80mm, this text is "
        ...
        {
            final RtfTableRow r = tbl.newTableRow();
            r.newTableCell(80 * MM_TO_TWIPS).newParagraph()
                    .newText("cell 2,0, width 80mm, this row has two cells.");
            r.newTableCell(80 * MM_TO_TWIPS).newParagraph()
                    .newText("cell 2,1, width 80mm, last cell.");
        }

    }

    private void secondTestTable(final RtfSection sect) throws IOException {
        sect.newParagraph().newText(
                "Second test: table with two nested tables in cell 1,1");
        final RtfTable tbl = sect.newTable(new DummyTableColumnsInfo());
        // first row, normal
        {
            final RtfTableRow r = tbl.newTableRow();
            final RtfTableCell c = r.newTableCell(160 * MM_TO_TWIPS);
            c.newParagraph().newText(
                    "second test table: cell 0,0, width 160mm, "
    ...
    private void thirdTestTable(final RtfSection sect) throws IOException {
        sect.newParagraph().newText(
                "Third test: table with two nested tables "
        ...
        // first row, normal
        {
            ...
            c.newParagraph()
                    .newText(
                            "third test table: cell 0,0, width 40mm, "
    ...
    private void fillNestedTable(final RtfTable tbl, final int index)
            throws IOException {
        final String id = "TABLE " + index;
        {
            final RtfTableRow r = tbl.newTableRow();
            r.newTableCell(80 * MM_TO_TWIPS)
                    .newParagraph()
                    .newText(
                            id
                                    + ":nested cell 0,0. Nested table contains 3 rows with 1,2 and 3 cells respectively");
        }

        {
            final RtfTableRow r = tbl.newTableRow();
            r.newTableCell(40 * MM_TO_TWIPS).newParagraph()
                    .newText(id + ":nested cell 1,0, 40mm.");
            r.newTableCell(40 * MM_TO_TWIPS).newParagraph()
                    .newText(id + ":nested cell 1,1, 40mm.");
        }

        {
            final RtfTableRow r = tbl.newTableRow();
            r.newTableCell(30 * MM_TO_TWIPS).newParagraph()
                    .newText(id + ":nested cell 2,0, 30mm.");
            r.newTableCell(30 * MM_TO_TWIPS).newParagraph()
                    .newText(id + ":nested cell 2,1, 30mm.");
            r.newTableCell(20 * MM_TO_TWIPS).newParagraph()
                    .newText(id + ":nested cell 2,2, 20mm.");
        }
    }
}
