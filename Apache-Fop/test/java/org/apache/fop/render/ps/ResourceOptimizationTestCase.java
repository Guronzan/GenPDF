...
public class ResourceOptimizationTestCase extends AbstractPostScriptTest {
    ...
    public void testResourceOptimization() throws Exception {
        ...
        // Prepare output file
        final File outputFile = renderFile(ua, "ps-resources.fo", "-if-l"
                + handler.getPSUtil().getLanguageLevel());
        verifyPostScriptFile(outputFile);
    }

    private void verifyPostScriptFile(final File psFile) throws IOException,
            DSCException {
        ...
        try {
            ...
            final PSResource form1 = new PSResource(PSResource.TYPE_FORM,
                    "FOPForm:1");
            final PSResource helvetica = new PSResource(PSResource.TYPE_FONT,
                    "Helvetica");
            final PSResource helveticaBold = new PSResource(
                    PSResource.TYPE_FONT, "Helvetica-Bold");
            ...
            final DSCCommentDocumentSuppliedResources supplied = (DSCCommentDocumentSuppliedResources) gotoDSCComment(
            ...
            final DSCCommentDocumentNeededResources needed = (DSCCommentDocumentNeededResources) gotoDSCComment(
                    parser, DSCConstants.DOCUMENT_NEEDED_RESOURCES);
            resources = needed.getResources();
            assertEquals(2, resources.size());
            assertTrue("Expected Helvetica as needed resource",
                    resources.contains(new PSResource(PSResource.TYPE_FONT,
                            "Helvetica")));
            ...
            final Collection strings = new java.util.HashSet(
                    Arrays.asList(new String[] { "Helvetica", "Helvetica-Bold" }));
            for (int i = 0; i < 2; i++) {
                include = (DSCCommentIncludeResource) gotoDSCComment(parser,
                        DSCConstants.INCLUDE_RESOURCE);
            ...
            // ---=== Page 1 ===---
            DSCCommentPage page = (DSCCommentPage) gotoDSCComment(parser,
                    DSCConstants.PAGE);
            assertEquals(1, page.getPagePosition());

            assertEquals(DSCAtend.class,
                    gotoDSCComment(parser, DSCConstants.PAGE_RESOURCES)
                            .getClass());
            ...
            beginDocument = (DSCCommentBeginDocument) gotoDSCComment(parser,
                    DSCConstants.BEGIN_DOCUMENT);
            assertEquals("test/resources/images/barcode.eps", beginDocument
                    .getResource().getName());
            final DSCListener listener = new DefaultNestedDocumentHandler(null);
            ...
            // No headers in between, as they should have been put at the
            // beginning of the file
            assertEquals(DSCCommentEndOfFile.class, parser.nextEvent()
                    .asDSCComment().getClass());
