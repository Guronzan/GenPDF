...
/**
 * Tests that RowGroupBuilder returns, for each part of a table, the expected
 * number of row-groups with the expected number or rows in each.
 */
public class RowGroupBuilderTestCase extends AbstractTableTest {

    /**
     * Checks that the given table-body(header,footer) will return row groups as
     * expected. More precisely, checks that the number of row groups
     * corresponds to the size of the given array, and that the number of rows
     * inside each row group is equal to the corresponding integer in the array.
     *
     * @param part
     *            a table part whose row groups are to be checked
     * @param expectedRowLengths
     *            expected lengths of all the row groups of this part of the
     *            table
     */
    private void checkTablePartRowGroups(final TablePart part,
            final int[] expectedRowLengths) {
        final Iterator rowGroupIter = part.getRowGroups().iterator();
        for (int i = 0; i < expectedRowLengths.length; i++) {
            assertTrue(rowGroupIter.hasNext());
            final List rowGroup = (List) rowGroupIter.next();
     ...
     * 
     * @param tableIter
     *            an iterator over the tables to check
     * @param expectedHeaderRowLengths
     *            expected row-group sizes for the header. If null the table is
     *            not expected to have a header
     * @param expectedFooterRowLengths
     *            expected row-group sizes for the footer. If null the table is
     *            not expected to have a footer
     * @param expectedBodyRowLengths
     *            expected row-group sizes for the body(-ies)
     */
    private void checkNextTableRowGroups(final Iterator tableIter,
            final int[] expectedHeaderRowLengths,
            final int[] expectedFooterRowLengths,
            final int[][] expectedBodyRowLengths) {
        final Table table = (Table) tableIter.next();
        if (expectedHeaderRowLengths == null) {
            assertNull(table.getTableHeader());
        } else {
            checkTablePartRowGroups(table.getTableHeader(),
                    expectedHeaderRowLengths);
        }
        ...
        final Iterator bodyIter = table.getChildNodes();
    ...
    public void checkSimple(final String filename) throws Exception {
        setUp(filename);
        ...
        checkNextTableRowGroups(tableIter, new int[] { 1 }, null,
                new int[][] { { 1 } });

        // Table 6: no header, one footer (1 row), one body (1 row)
        checkNextTableRowGroups(tableIter, null, new int[] { 1 },
                new int[][] { { 1 } });

        // Table 7: one header (1 row), one footer (1 row), one body (1 row)
        checkNextTableRowGroups(tableIter, new int[] { 1 }, new int[] { 1 },
                new int[][] { { 1 } });

        // Table 8: one header (2 rows), one footer (3 rows), one body (2 rows)
        checkNextTableRowGroups(tableIter, new int[] { 1, 1 }, new int[] { 1,
                1, 1 }, new int[][] { { 1, 1 } });

        // Table 9: one header (3 rows), one footer (2 rows), three bodies (2
        // rows, 1 row, 3 rows)
        checkNextTableRowGroups(tableIter, new int[] { 1, 1, 1 }, new int[] {
                1, 1 }, new int[][] { { 1, 1 }, { 1 }, { 1, 1, 1 } });
    }

    public void checkSpans(final String filename) throws Exception {
        setUp(filename);
        final Iterator tableIter = getTableIterator();
        ...
        // Table 3: no header, no footer, one body (1 row-group of 2 rows, 1
        // row)
        checkNextTableRowGroups(tableIter, null, null, new int[][] { { 2, 1 } });

        // Table 4: no header, no footer, one body (1 row, 1 row-group of 2
        // rows)
        checkNextTableRowGroups(tableIter, null, null, new int[][] { { 1, 2 } });

        // Table 5: no header, no footer, one body (1 row, 1 row-group of 3
        // rows, 1 row)
        checkNextTableRowGroups(tableIter, null, null,
                new int[][] { { 1, 3, 1 } });

        // Table 6: one header (1 row-group of 2 rows), one footer (1 row, 1
        // row-group of 3 rows),
        // one body (1 row-group of 2 rows, 1 row, 1 row-group of 3 rows)
        checkNextTableRowGroups(tableIter, new int[] { 2 }, new int[] { 1, 3 },
                new int[][] { { 2, 1, 3 } });
    }
