...
/* $Id: BitmapComparator.java 679326 2008-07-24 09:35:34Z vhennebert $ */
 ...
 * org.apache.batik.test.util.ImageCompareText (Apache Batik) into this separate
 * class.
 * <p>
...
public class BitmapComparator {
     ...
     * @param gen
     *            the newly generated bitmap
     * @return the diff bitmap
     */
    public static BufferedImage buildDiffImage(BufferedImage ref,
            BufferedImage gen) {
        final BufferedImage diff = new BufferedImage(ref.getWidth(),
                ref.getHeight(), BufferedImage.TYPE_INT_ARGB);
        final WritableRaster refWR = ref.getRaster();
        final WritableRaster genWR = gen.getRaster();
        final WritableRaster dstWR = diff.getRaster();
    ...
    /**
     * Builds a combined image that places a number of images next to each other
     ...
     */
    public static BufferedImage buildCompareImage(final BufferedImage[] images) {
        final BufferedImage cmp = new BufferedImage(images[0].getWidth()
                * images.length, images[0].getHeight(),
                BufferedImage.TYPE_INT_ARGB);

        final Graphics2D g = cmp.createGraphics();
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            if (images[i] != null) {
                ...
                lastWidth = 20; // Maybe add a special placeholder image instead
                                // later
            }
        }
        g.dispose();
     ...
     * @return the combined image
     */
    public static BufferedImage buildCompareImage(final BufferedImage ref,
            final BufferedImage gen) {
     ...
     * @return the bitmap as BufferedImage TODO This method doesn't close the
     *         InputStream opened by the URL.
     */
    public static BufferedImage getImage(final URL url) {
        final ImageTagRegistry reg = ImageTagRegistry.getRegistry();
        final Filter filt = reg.readURL(new ParsedURL(url));
        if (filt == null) {
            return null;
        }

        final RenderedImage red = filt.createDefaultRendering();
        if (red == null) {
            return null;
        }

        final BufferedImage img = new BufferedImage(red.getWidth(),
                red.getHeight(), BufferedImage.TYPE_INT_ARGB);
     ...
     */
    public static BufferedImage getImage(final File bitmapFile) {
        try {
            ...
            try {
                ...
                final ImageTagRegistry reg = ImageTagRegistry.getRegistry();
                final Filter filt = reg.readStream(in);
                if (filt == null) {
                    return null;
                }
