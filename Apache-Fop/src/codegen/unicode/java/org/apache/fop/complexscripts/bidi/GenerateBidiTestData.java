...
/**
 * <p>
 * Utility for generating a Java class and associated data files representing
 * bidirectional confomance test data from the Unicode Character Database and
 * Unicode BidiTest data files.
 * </p>
 *
 * <p>
...
public final class GenerateBidiTestData {
     ...
     *
     * @param ucdFileName
     *            name (as URL) of file containing unicode character database
     *            data
     * @param bidiFileName
     *            name (as URL) of file containing bidi test data
     * @param outFileName
     *            name of the output class file
     * @throws Exception
     */
    private static void convertBidiTestData(final String ucdFileName,
        ...
        out.println("    public static final int NUM_TEST_SEQUENCES = "
                + numTestSpecs + ";");
        out.println();
     ...
     *
     * @param ucdFileName
     *            name (as URL) of unicode character database data
     */
    private static void readBidiTypeData(final String ucdFileName)
            throws Exception {
        final BufferedReader b = new BufferedReader(new InputStreamReader(
                new URL(ucdFileName).openStream()));
        String line;
        final int n;
        // singleton map - derived from single char entry
        final Map/* <Integer,List> */sm = new HashMap/* <Integer,List> */();
        // interval map - derived from pair of block endpoint entries
        final Map/* <String,int[3]> */im = new HashMap/* <String,int[3]> */();
        if (verbose) {
        ...
        final List tdl = processTypeData(sm, im, new ArrayList());
        // dump instrumentation
        if (verbose) {
            log.info("");
            log.info("Read type ranges : " + numTypeRanges);
            log.info("Read lines       : " + lineNumber);
        }
    ...
    private static void parseTypeProperties(final String line, final Map/*
        ...
        if (sa.length >= 5) {
            ...
            final int bc = parseBidiClassAny(sa[4]);
            if (bc >= 0) {
                final String ucName = sa[1];
                if (isBlockStart(ucName)) {
                    ...
                    final String ucBlock = getBlockName(ucName);
                    if (im.containsKey(ucBlock)) {
                        final int[] ba = (int[]) im.get(ucBlock);
                        assert ba.length == 3;
                        if (ba[1] < 0) {
                            ba[1] = uc;
                        } else {
                ...
                throw new IllegalArgumentException("invalid bidi class '"
                        + sa[4] + "' at entry: " + line);
            }
        } else {
            throw new IllegalArgumentException(
                    "invalid unicode character database entry: " + line);
        }
    }

    private static boolean isBlockStart(final String s) {
        return s.startsWith("<") && s.endsWith("First>");
    }

    private static boolean isBlockEnd(final String s) {
        return s.startsWith("<") && s.endsWith("Last>");
    }

    private static String getBlockName(final String s) {
        final String[] sa = s.substring(1, s.length() - 1).split(",");
        assert sa != null && sa.length > 0;
        return sa[0].trim();
    }

    private static List processTypeData(final Map/* <Integer,List> */sm,
            final Map/* <String,int[3]> */im, final List tdl) {
        for (int i = BidiConstants.FIRST, k = BidiConstants.LAST; i <= k; i++) {
            final Map/* <Integer,Integer> */rm = new TreeMap/* <Integer,Integer> */();
            // populate intervals from singleton map
            final List sl = (List) sm.get(Integer.valueOf(i));
            if (sl != null) {
                for (final Iterator it = sl.iterator(); it.hasNext();) {
                    final Integer s = (Integer) it.next();
                    final int uc = s.intValue();
                    rm.put(Integer.valueOf(uc), Integer.valueOf(uc + 1));
                }
            }
            // populate intervals from (block) interval map
            if (!im.isEmpty()) {
                for (final Iterator it = im.values().iterator(); it.hasNext();) {
                    final int[] ba = (int[]) it.next();
                    assert ba != null && ba.length > 2;
                    if (ba[2] == i) {
                        rm.put(Integer.valueOf(ba[0]),
                                Integer.valueOf(ba[1] + 1));
                    }
                }
            }
    ...
    private static List extractRanges(final Map/* <Integer,Integer> */rm) {
     ...
     *
     * @param bidiFileName
     *            name (as URL) of bidi test data
     */
    private static void readBidiTestData(final String bidiFileName)
            throws Exception {
        final BufferedReader b = new BufferedReader(new InputStreamReader(
                new URL(bidiFileName).openStream()));
        String line;
        int n;
        final List tdl = new ArrayList();
        final List ldl = new ArrayList();
        if (verbose) {
        ...
        if (verbose) {
            ...
            log.info("Read level specs : " + numLevelSpecs);
            log.info("Read test specs  : " + numTestSpecs);
            log.info("Read lines       : " + lineNumber);
        }
        if (!ignoreDeprecatedTypeData) {
            td = (int[][]) tdl.toArray(new int[tdl.size()][]);
    ...
    private static int readLevels(String line, final BufferedReader b,
    ...
    private static int testPrefix(final BufferedReader b, final String pfx)
            throws IOException {
        int rv = 0;
    ...
    private static int[] parseType(final List lines) {
        if (lines != null && lines.size() >= 1) {
            final String line = (String) lines.get(0);
            if (line.startsWith(PFX_TYPE)) {
                // @Type: BIDI_CLASS ':' LWSP CHARACTER_CLASS
                final String[] sa = line.split(":");
                if (sa.length == 3) {
                    final String bcs = sa[1].trim();
                    final String crs = sa[2].trim();
                    final int bc = parseBidiClass(bcs);
                    final List rl = parseCharacterRanges(crs);
    ...
    private static int[] createTypeData(final int bc, final List ranges) {
        final int[] data = new int[1 + 2 * ranges.size()];
        int k = 0;
        data[k++] = bc;
        for (final Iterator it = ranges.iterator(); it.hasNext();) {
            final int[] r = (int[]) it.next();
            data[k++] = r[0];
            data[k++] = r[1];
        }
        return data;
    ...
    private static int parseBidiClassAny(final String bidiClass) {
        try {
    ...
    private static List parseCharacterRanges(final String charRanges) {
        List ranges = new ArrayList();
        final CharacterIterator ci = new StringCharacterIterator(charRanges);
        ...
        while (more) {
            ...
            if ((s = maybeReadSeparator(ci)) != null) {
                if (s.length() != 0 && !s.equals("||")) {
                    badRangeSpec("invalid item separator \"" + s + "\"",
                            charRanges);
                }
            }
            // read item
    ...
    private static boolean atEnd(final CharacterIterator ci) {
        return ci.getIndex() >= ci.getEndIndex();
    }

    private static boolean readStartOfList(final CharacterIterator ci) {
        return maybeReadNext(ci, '[');
    }

    private static void skipSpace(final CharacterIterator ci) {
        while (!atEnd(ci)) {
    ...
    private static boolean maybeReadNext(final CharacterIterator ci,
            final char next) {
        while (!atEnd(ci)) {
            final char c = ci.current();
            if (c == next) {
    ...
    private static int[] maybeReadItem(final CharacterIterator ci) {
        ...
        if (p2 < 0) {
            return new int[] { p1, p1 + 1 }; // convert to half open interval [
            // P1, P1+1 )
        } else if (p1 <= p2) {
            return new int[] { p1, p2 + 1 }; // convert to half open interval [
            // P1, P2+2 )
        } else {
            badRangeSpec(
                    "invalid item range, second item must be greater than or equal to first item",
                    ci);
            return null;
        }
    }

    private static int maybeReadCodePoint(final CharacterIterator ci) {
        if (maybeReadNext(ci, '\\')) {
            if (maybeReadNext(ci, 'u')) {
                final String s = maybeReadHexDigits(ci, 4);
                if (s != null) {
                    return Integer.parseInt(s, 16);
                } else {
                    badRangeSpec(
                            "incomplete escaped code point, requires 4 hex digits",
                            ci);
                }
            } else if (maybeReadNext(ci, 'U')) {
                final String s = maybeReadHexDigits(ci, 8);
                if (s != null) {
                    return Integer.parseInt(s, 16);
                } else {
                    badRangeSpec(
                            "incomplete escaped code point, requires 8 hex digits",
                            ci);
                }
            } else {
                final char c = ci.current();
    ...
    private static String maybeReadSeparator(final CharacterIterator ci) {
        if (maybeReadNext(ci, '|')) {
            if (maybeReadNext(ci, '|')) {
                return "||";
            } else {
    ...
    private static boolean readEndOfList(final CharacterIterator ci) {
        return maybeReadNext(ci, ']');
    }

    private static List complementRanges(final List ranges) {
        final Map/* <Integer,Integer> */rm = new TreeMap/* <Integer,Integer> */();
        for (final Iterator it = ranges.iterator(); it.hasNext();) {
            final int[] r = (int[]) it.next();
    ...
    private static List removeSurrogates(final List ranges) {
        final List rsl = new ArrayList(ranges.size());
        for (final Iterator it = ranges.iterator(); it.hasNext();) {
            final int[] r = (int[]) it.next();
     ...
     */
    private static boolean intersectsRange(final int[] r1, final int[] r2) {
    ...
    /**
     * Remove range r2 from range r1, leaving zero, one, or two remaining
     * ranges.
     */
    private static List removeRange(final int[] r1, final int[] r2) {
        final List rl = new ArrayList();
        if (r1[1] <= r2[0]) { // r1 precedes r2 or abuts r2 on right
            rl.add(r1);
        } else if (r1[0] >= r2[1]) { // r2 precedes r1 or abuts r1 on left
            rl.add(r1);
    ...
    private static void badRangeSpec(final String reason,
            final String charRanges) throws IllegalArgumentException {
        if (verbose) {
            log.info("");
        }
        throw new IllegalArgumentException("bad range specification: " + reason
                + ": \"" + charRanges + "\"");
    }

    private static void badRangeSpec(final String reason,
            final CharacterIterator ci) throws IllegalArgumentException {
        if (verbose) {
            log.info("");
        }
        throw new IllegalArgumentException("bad range specification: " + reason
                + ": starting at \"" + remainder(ci) + "\"");
    }
     ...
     * LEVEL_SPEC \n REORDER_SPEC \n ( TEST_SPEC \n )+
     */
    private static int[] parseLevels(final List lines) {
        int[] la = null; // levels array
        int[] ra = null; // reorder array
        final List tal = new ArrayList();
            ...
            for (final Iterator it = lines.iterator(); it.hasNext();) {
                ...
                if (line.startsWith(PFX_LEVELS)) {
                    if (la == null) {
                        ...
                        throw new IllegalArgumentException(
                                "redundant levels array: \"" + line + "\"");
                    }
                } else if (line.startsWith(PFX_REORDER)) {
                    if (la == null) {
                        throw new IllegalArgumentException(
                                "missing levels array before: \"" + line + "\"");
                    } else if (ra == null) {
                        ra = parseReorderSpec(line, la);
                    } else {
                        throw new IllegalArgumentException(
                                "redundant reorder array: \"" + line + "\"");
                    }
                } else if (la != null && ra != null) {
                    final int[] ta = parseTestSpec(line, la);
                    if (ta != null) {
                        if (verbose) {
                    ...
                    throw new IllegalArgumentException(
                            "missing reorder array before: \"" + line + "\"");
                }
            }
        }
        if (la != null && ra != null) {
    ...
    private static int[] createLevelData(final int[] la, final int[] ra,
            final List tal) {
        final int nl = la.length;
        final int[] data = new int[1 + nl * 2 + (nl + 1) * tal.size()];
        int k = 0;
        data[k++] = nl;
        for (int i = 0, n = nl; i < n; i++) {
            data[k++] = la[i];
        }
        final int nr = ra.length;
        for (int i = 0, n = nr; i < n; i++) {
            data[k++] = ra[i];
        }
        for (final Iterator it = tal.iterator(); it.hasNext();) {
            final int[] ta = (int[]) it.next();
            if (ta == null) {
                throw new IllegalStateException("null test array");
            } else if (ta.length == nl + 1) {
                for (final int element : ta) {
                    data[k++] = element;
                }
            } else {
     ...
     */
    private static int[] parseLevelSpec(final String line) {
        final CharacterIterator ci = new StringCharacterIterator(line);
    ...
    private static Integer maybeReadInteger(final CharacterIterator ci) {
    ...
    private static boolean maybeReadToken(final CharacterIterator ci,
            final String s) {
        final int startIndex = ci.getIndex();
    ...
    private static void badLevelSpec(final String reason,
            final CharacterIterator ci) throws IllegalArgumentException {
        if (verbose) {
            log.info("");
        }
        throw new IllegalArgumentException("bad level specification: " + reason
                + ": starting at \"" + remainder(ci) + "\"");
    }

    private static int[] createLevelsArray(final List levels) {
        final int[] la = new int[levels.size()];
        int k = 0;
     ...
     * @Reorder: ( LWSP NUMBER )*
     */
    private static int[] parseReorderSpec(final String line, final int[] levels) {
        final CharacterIterator ci = new StringCharacterIterator(line);
        final List rl = new ArrayList();
        // read prefix
        skipSpace(ci);
    ...
    private static void badReorderSpec(final String reason,
            final CharacterIterator ci) throws IllegalArgumentException {
        if (verbose) {
            log.info("");
        }
        throw new IllegalArgumentException("bad reorder specification: "
                + reason + ": starting at \"" + remainder(ci) + "\"");
    }

    private static int[] createReorderArray(final List reorders,
            final int[] levels) {
        final int nr = reorders.size();
        final int nl = levels.length;
        if (nr <= nl) {
            final int[] ra = new int[nl];
            final Iterator it = reorders.iterator();
            for (int i = 0, n = nl; i < n; i++) {
                int r = -1;
                if (levels[i] >= 0) {
                    if (it.hasNext()) {
            ...
            throw new IllegalArgumentException(
                    "excessive number of reorder array entries, expected no more than "
                            + nl + ", but got " + nr + " entries");
        }
    }
     ...
     */
    private static int[] parseTestSpec(final String line, final int[] levels) {
        final CharacterIterator ci = new StringCharacterIterator(line);
    ...
    private static int maybeReadBidiClass(final CharacterIterator ci) {
        int bc = -1;
        final int i = ci.getIndex();
        String s;
    ...
    private static void badTestSpec(final String reason,
            final CharacterIterator ci) throws IllegalArgumentException {
        if (verbose) {
            log.info("");
        }
        throw new IllegalArgumentException("bad test specification: " + reason
                + ": starting at \"" + remainder(ci) + "\"");
    }

    private static int[] createTestArray(final List classes, final int bitset,
            final int[] levels) {
        final int nc = classes.size();
        if (nc <= levels.length) {
            final int[] ta = new int[1 + nc];
            int k = 0;
            ta[k++] = bitset;
            ...
            throw new IllegalArgumentException(
                    "excessive number of test array entries, expected no more than "
                            + levels.length + ", but got " + nc + " entries");
        }
    }
     ...
     *
     * @param out
     *            - bidi test data java class file print writer
     * @param outFileName
     *            - (full path) name of bidi test data java class file
     */
    private static void dumpData(final PrintWriter out, final String outFileName)
            throws IOException {
        final File f = new File(outFileName);
        final File p = f.getParentFile();
        if (td != null) {
            final String pfxTD = "TD";
            dumpResourcesDescriptor(out, pfxTD, td.length);
            dumpResourcesData(p, f.getName(), pfxTD, td);
        }
        if (ld != null) {
            final String pfxTD = "LD";
            dumpResourcesDescriptor(out, pfxTD, ld.length);
            dumpResourcesData(p, f.getName(), pfxTD, ld);
        }
    }

    private static void dumpResourcesDescriptor(final PrintWriter out,
            final String prefix, final int numResources) {
        out.println("    public static final String " + prefix + "_PFX = \""
                + prefix + "\";");
        out.println("    public static final int " + prefix + "_CNT = "
                + numResources + ";");
        out.println("");
    }
    ...
    private static String extractDataFileName(final String btcName) {
    ...
    public static void main(final String[] args) {
        ...
        if (!ok) {
            log.info("Usage: GenerateBidiTestData [-v] [-i] [-d <ucdFile>] [-b <bidiFile>] [-o <outputFile>]");
            log.info("  defaults:");
            if (ignoreDeprecatedTypeData) {
                log.info("    <ucdFile>    : " + ucdFileName);
            }
            ...
            try {
                ...
                log.info("An unexpected error occured at line: " + lineNumber);
                e.printStackTrace();
            }
        }
    }
