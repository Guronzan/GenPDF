...
public class AdobeStandardEncodingTestCase {
    ...
    public static void setupReader() throws FileNotFoundException {
        final InputStream inStream = AdobeStandardEncodingTestCase.class
                .getResourceAsStream("AdobeStandardEncoding.txt");
        adobeStandardEncoding = new BufferedReader(new InputStreamReader(
                inStream));
    }

    /**
     * Probably the best way to test the encoding is by converting it back to
     * format specified in the file, that way we can ensure data has been
     * migrated properly.
     *
     * @throws IOException
    ...
    public void testCorrectEncoding() throws IOException {
        for (final AdobeStandardEncoding encoding : AdobeStandardEncoding
                .values()) {
            final String expectedLine = getLine();
            final String hexUnicode = toHexString(encoding.getUnicodeIndex(), 4);
            final String hexAdobe = toHexString(encoding.getAdobeCodePoint(), 2);
            final String actualLine = hexUnicode + "\t" + hexAdobe + "\t# "
    ...
    private String toHexString(final int number, final int length) {
