...
AbstractNamedAFPObjectTest<IncludeObject> {
    ...
    public void testWriteToStream() throws IOException {
        ...
        final byte[] expected = defaultIncludeObjectBytes(
                sut.getTripletDataLength(), sut.getNameBytes());
    ...
    private void testWriteToStreamHelper(final IncludeObject sut,
    ...
    private byte[] defaultIncludeObjectBytes(final int tripletDataLength,
            final byte[] nameData) {

        final byte[] expected = new byte[36];

        final byte[] header = new byte[] { 0x5A, // Structured field identifier
                0x00, // Length byte 1
                0x10, // Length byte 2
        ...
        final byte[] lengthBytes = BinaryUtils.convert(35 + tripletDataLength,
                2); // Ignore first byte
    ...
    private static void writeOsetTo(final byte[] out, final int offset,
            final int oset) {
        if (oset > -1) {
            final byte[] y = BinaryUtils.convert(oset, 3);
            out[offset] = y[0];
            out[offset + 1] = y[1];
            out[offset + 2] = y[2];
        } else {
