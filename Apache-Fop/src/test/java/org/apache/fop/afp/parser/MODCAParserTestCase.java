...
public class MODCAParserTestCase {
    ...
    public void testReadNextStructuredField1() throws Exception {
        ...
        final byte[][] goodInputStream = new byte[][] { new byte[] { CARRIAGE_CONTROL_CHAR } };

        for (final byte[] b : goodInputStream) {
            try {
        ...
        final byte[][] badInputStream = new byte[][] { new byte[] {},
                new byte[] { CARRIAGE_RETURN }, new byte[] { LINE_FEED } };

        for (final byte[] b : badInputStream) {
            final UnparsedStructuredField usf = new MODCAParser(
                    new ByteArrayInputStream(b)).readNextStructuredField();
            assertNull(usf);
        }
    }
     ...
     * UnparsedStructuredField from a well formed structured field
     *
     * @throws Exception
     *             *
     */
    ...
    public void testReadNextStructuredField2() throws Exception {
        ...
        testSF((byte) 0xd3, (byte) 0xa8, (byte) 0x89, // SFTypeID
                ...
                false, false, new byte[] { 0, 0 }, new byte[] { 1 }, null);

        // with extension data
        testSF((byte) 0xd3, (byte) 0xa8,
                (byte) 0x89, // SFTypeID
                (byte) 0, // flags excluding the bits for
                // extension present, segmented data and padding present
                false, false, new byte[] { 0, 0 }, new byte[] { 1 },
                new byte[] { 10 });
        ...
        testSF((byte) 0xd3, (byte) 0xa8, (byte) 0x89, // SFTypeID
                (byte) 0, // flags excluding the bits for
                // extension present, segmented data and padding present
                false, false, new byte[] { 1, 2 }, new byte[] { 1 }, null);

        // with padding present and segmented data
        testSF((byte) 0xd3, (byte) 0xa8, (byte) 0x89, // SFTypeID
                (byte) (1 << 3), // flags excluding the bits for
                // extension present, segmented data and padding present
                true, true, new byte[] { 0, 0 }, new byte[] { 1 }, null);
    ...
    private void testSF(final byte classCode, final byte typeCode,
            final byte categoryCode, byte flags, final boolean segmentedData,
            final boolean paddingPresent, final byte[] reserved,
            final byte[] data, final byte[] extData) throws Exception {

        byte extDataLength = 0;
        final boolean extensionPresent = extData != null;
        ...
        final short length = (short) (INTRODUCER_LENGTH + data.length + extDataLength);
        final byte[] lengthBytes = new byte[] { (byte) (length >> 8),
                (byte) (length & 0xFF) };

        final byte[] sfBytes = new byte[length];
        ...
        final byte[] delimiteredSF = new byte[length + 1];

        delimiteredSF[0] = (byte) 0x5A;

        System.arraycopy(sfBytes, 0, delimiteredSF, 1, length);

        final InputStream bis = new ByteArrayInputStream(delimiteredSF);

        final UnparsedStructuredField actual = new MODCAParser(bis)
                .readNextStructuredField();
        ...
        final byte[] introducerData = new byte[] { (byte) (length >> 8),
                (byte) (length & 0xFF), classCode, typeCode, categoryCode,
                flags, reserved[0], reserved[1] };
        ...
        assertEquals(extData == null ? 0 : extData.length + 1, // 1 byte for
                                                               // length byte
                actual.getExtLength());

        assertTrue(Arrays.equals(data, actual.getData()));

        final int expectedSfTypeID = (classCode & 0xFF) << 16
                | (typeCode & 0xFF) << 8 | categoryCode & 0xFF;
