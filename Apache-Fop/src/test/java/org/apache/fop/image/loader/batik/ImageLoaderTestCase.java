...
public class ImageLoaderTestCase {
    ...
    public void testSVG() throws Exception {
        ...
        final ImageXMLDOM imgDom = (ImageXMLDOM) img;
        ...
        if (DEBUG_TARGET_DIR != null) {
            ImageWriterUtil.saveAsPNG(imgRed.getRenderedImage(),
                    (int) userAgent.getTargetResolution(), new File(
                            DEBUG_TARGET_DIR, "out.svg.png"));
        }
    ...
    public void testSVGNoViewbox() throws Exception {
        final String uri = "test/resources/images/circles.svg";

        final FopFactory ff = FopFactory.newInstance();
        ff.setSourceResolution(96);
        ff.setTargetResolution(300);

        final FOUserAgent userAgent = ff.newFOUserAgent();

        final ImageManager manager = ff.getImageManager();
        ImageInfo info = manager.preloadImage(uri,
                userAgent.getImageSessionContext());
        assertNotNull("ImageInfo must not be null", info);
        ...
        if (DEBUG_TARGET_DIR != null) {
            ImageWriterUtil.saveAsPNG(imgRed.getRenderedImage(),
                    (int) userAgent.getTargetResolution(), new File(
                            DEBUG_TARGET_DIR, "circles.svg.png"));
        }
    ...
    public void testSVGWithReferences() throws Exception {
        final String uri = "test/resources/fop/svg/images.svg";
        final FopFactory ff = FopFactory.newInstance();
        final FOUserAgent userAgent = ff.newFOUserAgent();
        ...
        assertEquals(ImageFlavor.RENDERED_IMAGE, img.getFlavor());
        final ImageRendered imgRed = (ImageRendered) img;
        final RenderedImage renImg = imgRed.getRenderedImage();
        assertNotNull(renImg);
        if (DEBUG_TARGET_DIR != null) {
            ImageWriterUtil.saveAsPNG(renImg, (int) userAgent
                    .getTargetResolution(), new File(DEBUG_TARGET_DIR,
                    "images.svg.png"));
        }
        ...
        assertEquals(400000, info.getSize().getWidthMpt());
        assertEquals(400000, info.getSize().getHeightMpt());
        final Raster raster = renImg.getData();
        // This pixel is white
        final int[] pixel1 = raster.getPixel(1, 1, (int[]) null);
        // This pixel is from the embedded JPG and is not white
        final int[] pixel80 = raster.getPixel(80, 80, (int[]) null);
