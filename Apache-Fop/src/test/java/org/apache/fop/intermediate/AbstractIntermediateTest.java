...
public abstract class AbstractIntermediateTest {
     ...
     */
    public AbstractIntermediateTest(final File testFile) throws IOException {
    ...
    public void setUp() throws Exception {
        ...
        this.testDoc = testAssistant.loadTestCase(this.testFile);
        this.fopFactory = testAssistant.getFopFactory(this.testDoc);
        this.intermediate = buildIntermediateDocument(testAssistant
                .getTestcase2FOStylesheet());
        if (this.outputDir != null) {
            testAssistant.saveDOM(this.intermediate, new File(this.outputDir,
                    this.testFile.getName() + ".1"
     ...
     * @throws IOException
     *             if an IO error occurs while loading the schema
     * @throws SAXException
     *             if a SAX-related exception (including a validation error)
     *             occurs
     */
    ...
    protected FOUserAgent createUserAgent() {
        ...
        try {
            userAgent.setBaseURL(this.testDir.toURI().toURL().toExternalForm());
    ...
    protected void setupOutputDirectory() {
        final String s = System.getProperty("fop.intermediate.outdir");
        if (s != null && s.length() > 0) {
            this.outputDir = new File(s);
    ...
    public void testParserToIntermediateFormat() throws Exception {
        validate(this.intermediate);
        final Source src = new DOMSource(this.intermediate);
        final Document doc = parseAndRenderToIntermediateFormat(src);
        if (this.outputDir != null) {
            final File tgtFile = new File(this.outputDir,
                    this.testFile.getName() + ".2"
                            + getIntermediateFileExtension());
            testAssistant.saveDOM(doc, tgtFile);
        }

        XMLAssert.assertXMLEqual(this.intermediate, doc);
    }
     ...
     *             if an error occurs while processing the document
     */
    protected abstract Document parseAndRenderToIntermediateFormat(
            final Source src) throws Exception;
     ...
     * @throws Exception
     *             if an error occurs while rendering the document
     */
    protected abstract void parseAndRender(final Source src,
