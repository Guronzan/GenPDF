...
/* $Id: PSTTFGenerator.java 1353173 2012-06-23 18:45:21Z gadams $ */
...
/**
 * This is a wrapper for {@link PSGenerator} that contains some members specific
 * for streaming TrueType fonts to a PostScript document.
 */
public class PSTTFGenerator {
    ...
    /**
     * The buffer is used to store the font file in an array of hex-encoded
     * strings. Strings are limited to 65535 characters, string will start with
     * a newline, 2 characters are needed to hex-encode each byte.
     */
     ...
     * 
     * @param gen
     *            the PSGenerator to wrap
     */
    public PSTTFGenerator(final PSGenerator gen) {
        this.gen = gen;
    ...
    public void startString() throws IOException {
        // We need to reset the streamer so that it starts a new line in the PS
        // document
     ...
     *            byte[] a byte array
     * @param offset
     *            the position in the byte array where the streaming must start
     * @param length
     *            the number of bytes to stream. This MUST be less than
     *            {@link #MAX_BUFFER_SIZE} - 1 since strings are suffixed by
     *            '00' (see Section 4.2 of Adobe Technical Note #5012,
     *            <em>The Type 42 Font Format Specification</em>.).
     */
    public void streamBytes(final byte[] byteArray, final int offset,
            final int length) throws IOException {
        if (length > MAX_BUFFER_SIZE) {
            throw new UnsupportedOperationException(
                    "Attempting to write a string to a PostScript"
