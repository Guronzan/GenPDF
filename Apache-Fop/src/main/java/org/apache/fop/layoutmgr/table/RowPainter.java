...
class RowPainter {
     ...
     */
    private final List<Integer> rowOffsets = new ArrayList<Integer>();
    ...
    void endTablePart(final boolean lastInBody, final boolean lastOnPage) {
        ...
        if (this.tablePartBackground != null) {
            ...
            for (final Block backgroundArea : this.tablePartBackgroundAreas) {
    ...
    void handleTableContentPosition(final TableContentPosition tcpos) {
        ...
        for (final CellPart cellPart : tcpos.cellParts) {
    ...
    private void addAreasForCell(final PrimaryGridUnit pgu, final int startPos,
        ...
        if (log.isDebugEnabled()) {
            ...
            log.debug(" rowHeight={}, cellTotalHeight={}", rowHeight,
                    cellTotalHeight);
        }
    ...
    private int getRowOffset(final int rowIndex) {
        return this.rowOffsets.get(rowIndex - this.firstRowIndex).intValue();
    }
