...
/* $Id: ScaledBaselineTable.java 1293736 2012-02-26 02:29:01Z gadams $ */
 ...
 * class uses a coordinate system with its origin where the dominant baseline
 * intersects the start edge of the box. All measurements are in mpt.
 */
final class ScaledBaselineTable {
     ...
     */
    ScaledBaselineTable(final int altitude, final int depth, final int xHeight,
            final int dominantBaselineIdentifier, final WritingMode writingMode) {
    ...
    int getBaseline(final int baselineIdentifier) {
        ...
        default:
            throw new IllegalArgumentException(
                    String.valueOf(baselineIdentifier));
        }
        return offset;
    }
    ...
    ScaledBaselineTable deriveScaledBaselineTable(final int baselineIdentifier) {
        final ScaledBaselineTable bac = new ScaledBaselineTable(this.altitude,
                this.depth, this.xHeight, baselineIdentifier, this.writingMode);
        return bac;
    }
