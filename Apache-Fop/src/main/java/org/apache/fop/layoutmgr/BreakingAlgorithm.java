...
public abstract class BreakingAlgorithm {
    ...
    protected void considerLegalBreak(final KnuthElement element,
        ...
        if (log.isTraceEnabled()) {
            log.trace(
                    "considerLegalBreak() at {} ({} + {} - {} }, parts/lines: {} - {}",
                    elementIdx, this.totalWidth, this.totalStretch,
                    this.totalShrink, this.startLine, this.endLine);
            log.trace("\tCurrent active node list: {} {}",
                    this.activeNodeCount, this.toString("\t"));
        }
        ...
        for (int line = this.startLine; line < this.endLine; line++) {
            for (KnuthNode node = getNode(line); node != null; node = node.next) {
                ...
                if (log.isTraceEnabled()) {
                    log.trace("\tr={}, difference={}", r, difference);
                    log.trace("\tline={}", line);
                }
    ...
    private void addBreaks(final int line, final int elementIdx) {
        ...
        for (int i = 0; i <= 3; i++) {
            if (this.best.notInfiniteDemerits(i)
                ...
                if (log.isTraceEnabled()) {
                    log.trace(
                            "\tInsert new break in list of {} from fitness class {}",
                            this.activeNodeCount, FitnessClasses.NAMES[i]);
                }
