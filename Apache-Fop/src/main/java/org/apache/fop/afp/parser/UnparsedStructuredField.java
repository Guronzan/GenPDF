...
public final class UnparsedStructuredField {
     ...
     */
    UnparsedStructuredField(final Introducer introducer, final byte[] data,
            final byte[] extData) {
    ...
    public int getSfTypeID() {
        return (getSfClassCode() & 0xFF) << 16 | (getSfTypeCode() & 0xFF) << 8
    ...
    byte[] getExtData() {
        ...
        final byte[] rtn = new byte[this.extData.length];
    ...
    public byte[] getData() {
        ...
        final byte[] rtn = new byte[this.data.length];
    ...
    static final class Introducer {
        ...
        Introducer(final byte[] introducerData) throws IOException {

            this.introducerData = introducerData;

            // Parse the introducer; the 8 bytes have already been read from the
            // stream just
            // before, so we parse the introducer from the byte array
            final DataInputStream iis = new DataInputStream(
            ...
            // Flags
            final byte f = iis.readByte();

            this.extensionPresent = (f & 0x01) != 0;
            this.segmentedData = (f & 0x04) != 0;
            this.paddingPresent = (f & 0x10) != 0;
        ...
        public byte[] getIntroducerData() {
            final byte[] rtn = new byte[this.introducerData.length];
