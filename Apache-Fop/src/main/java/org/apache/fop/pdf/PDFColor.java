...
/**
 * PDF Color object. It is currently only used to hold the transparent color of
 * a masked bitmap image. And in this context, only RGB and Gray values are
 * used.
 * <p>
 * Use of this class is discouraged. {@link PDFColorHandler} is now used for
 * in-content color selection. For masked bitmaps, it may be wiser to switch to
 * {@link java.awt.Color} in the long run.
 */
public class PDFColor extends PDFPathPaint {
     ...
     *
     * 1. CMYK color 3. 'Normal' java.awt.Color (RGB case assumed or implicit
     * conversion to sRGB)
     *
     * @param col
    ...
    public PDFColor(final java.awt.Color col) {
        final ColorSpace cs = col.getColorSpace();
        if (cs != null && cs instanceof DeviceCMYKColorSpace) {
            // CMYK case
            this.colorSpace = new PDFDeviceColorSpace(
                    PDFDeviceColorSpace.DEVICE_CMYK);
            final float[] cmyk = col.getColorComponents(null);
            this.cyan = cmyk[0];
            this.magenta = cmyk[1];
            this.yellow = cmyk[2];
            this.black = cmyk[3];
    ...
    public String getColorSpaceOut(final boolean fillNotStroke) {
        ...
        if (this.colorSpace.getColorSpace() == PDFDeviceColorSpace.DEVICE_RGB) { // colorspace
                                                                                 // is
                                                                                 // RGB
