 ...
 * in the WEB-INF directory of a web application using "servlet-content:" URIs.
 */
public class ServletContextURIResolver implements URIResolver {
     ...
     */
    public ServletContextURIResolver(final ServletContext servletContext) {
    ...
    public Source resolve(final String href, final String base)
        ...
        if (href.startsWith(SERVLET_CONTEXT_PROTOCOL)) {
            return resolveServletContextURI(href
                    .substring(SERVLET_CONTEXT_PROTOCOL.length()));
        } else {
            if (base != null && base.startsWith(SERVLET_CONTEXT_PROTOCOL)
                    && href.indexOf(':') < 0) {
                final String abs = base + href;
    ...
    protected Source resolveServletContextURI(String path)
        ...
        try {
            final URL url = this.servletContext.getResource(path);
            ...
            throw new TransformerException(
                    "Error accessing resource using servlet context: " + path,
                    mfue);
        }
    }
}
