...
public final class IFUtil {
     ...
     */
    public static StringBuffer toString(final AffineTransform transform,
     ...
     * @return the StringBuffer passed to this method
     */
    public static StringBuffer toString(final AffineTransform[] transforms,
            final StringBuffer sb) {
     ...
     * a configurator object the configuration from the
     * {@link org.apache.fop.apps.FopFactory} will be used. Otherwise, a default
     * font configuration will be set up.
     * 
     ...
     * This utility method is used to provide backward compatibility in
     * implementations of IFPainter that have not yet been upgraded to the
     * general position adjustments format.
     * 
     * @param dp
     *            an array of 4-tuples, expressing [X,Y] placment adjustments
     *            and [X,Y] advancement adjustments, in that order (may be null)
     * @param count
     *            if <code>dp</code> is not null, then a count of dp values to
     *            convert
     * @return if <code>dp</code> is not null, then an array of adjustments to
     *         the current x position prior to rendering individual glyphs;
     *         otherwise, null
     */
    public static int[] convertDPToDX(final int[][] dp, final int count) {
        int[] dx;
        if (dp != null) {
            dx = new int[count];
     ...
     *         otherwise, null
     */
    public static int[] convertDPToDX(final int[][] dp) {
     ...
     * 
     * @param dx
     *            an array of adjustments to the current x position prior to
     *            rendering individual glyphs or null
     * @param count
     *            if <code>dx</code> is not null, then a count of dx values to
     *            convert
     * @return if <code>dx</code> is not null, then an array of 4-tuples,
     *         expressing [X,Y] placment adjustments and [X,Y] advancement
     *         adjustments, in that order; otherwise, null
     */
    public static int[][] convertDXToDP(final int[] dx, final int count) {
        int[][] dp;
        if (dx != null) {
            dp = new int[count][4];
            for (int i = 0, n = count; i < n; i++) {
     ...
     *         adjustments, in that order; otherwise, null
     */
    public static int[][] convertDXToDP(final int[] dx) {
        return convertDXToDP(dx, dx != null ? dx.length : 0);
    }

    /**
     * Determine if position adjustment is the identity adjustment, i.e., no
     * non-zero adjustment.
     * 
     * @param pa
     *            a 4-tuple, expressing [X,Y] placment and [X,Y] advance
     *            adjuustments (may be null)
     * @return true if <code>dp</code> is null or contains no non-zero
     *         adjustment
     */
    public static boolean isPAIdentity(final int[] pa) {
        if (pa == null) {
            return true;
        } else {
            for (int k = 0; k < 4; k++) {
    ...
    /**
     * Determine if position adjustments is the identity adjustment, i.e., no
     * non-zero adjustment.
     * 
     * @param dp
     *            an array of 4-tuples, expressing [X,Y] placment adjustments
     ...
     *         adjustment
     */
    public static boolean isDPIdentity(final int[][] dp) {
        if (dp == null) {
            return true;
        } else {
    ...
    /**
     * Determine if position adjustments comprises only DX adjustments as
     * encoded by {@link #convertDPToDX}. Note that if given a set of all all
     * zero position adjustments, both this method and {@link #isDPIdentity}
     * will return true; however, this method may return true when
     * {@link #isDPIdentity} returns false.
     * 
     * @param dp
     *            an array of 4-tuples, expressing [X,Y] placment adjustments
     *            and [X,Y] advancement adjustments, in that order (may be null)
     * @return true if <code>dp</code> is not null and contains only xPlaAdjust
     *         and xAdvAdjust values consistent with the output of
     *         {@link #convertDPToDX}.
     */
    public static boolean isDPOnlyDX(final int[][] dp) {
        if (dp == null) {
            return false;
        } else {
            for (int i = 0, n = dp.length; i < n; i++) {
    ...
    /**
     * Adjust a position adjustments array. If both <code>paDst</code> and
     * <code>paSrc</code> are non-null, then <code>paSrc[i]</code> is added to
     * <code>paDst[i]</code>.
     * 
     * @param paDst
     *            a 4-tuple, expressing [X,Y] placment and [X,Y] advance
     ...
     *            adjuustments (may be null)
     */
    public static void adjustPA(final int[] paDst, final int[] paSrc) {
     ...
     * @param dp
     *            an array of 4-tuples, expressing [X,Y] placment adjustments
     *            and [X,Y] advancement adjustments, in that order
     * @param offset
     *            starting offset from which to copy
     * @param count
     *            number of entries to copy
     * @return a deep copy of the count position adjustment entries start at
     *         offset
     */
    public static int[][] copyDP(final int[][] dp, final int offset,
            final int count) {
        if (dp == null || offset > dp.length || offset + count > dp.length) {
            throw new IllegalArgumentException();
        } else {
            final int[][] dpNew = new int[count][4];
            for (int i = 0, n = count; i < n; i++) {
                final int[] paDst = dpNew[i];
                final int[] paSrc = dp[i + offset];
                for (int k = 0; k < 4; k++) {
                    paDst[k] = paSrc[k];
                }
            }
