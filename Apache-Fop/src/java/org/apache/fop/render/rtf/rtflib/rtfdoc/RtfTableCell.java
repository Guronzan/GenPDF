...
public class RtfTableCell extends RtfContainer implements
    ...
    /**
     * The table cell decides whether or not a newly added paragraph break will
     * be used to write the cell-end control word. For nested tables it is not
     * necessary.
     *
     * @param parBreak
     ...
     *            If it is deeper inside the whole cell it will be used, and if
     *            there is something on the same level (depth) it is also set
     *            because the method is called for all breaks in the correct
     *            order.
     */
    public void setLastParagraph(final RtfParagraphBreak parBreak,
            final int breakDepth) {
        if (parBreak != null && breakDepth >= this.lastBreakDepth) {
            this.lastBreak = parBreak;
            this.lastBreakDepth = breakDepth;
        }
    }

    /**
     * The last paragraph break was just stored before, now the control word is
     * really switched
     */
    public void finish() {
        // If it is nested and contains another table do not set it
        if (getRow().getTable().isNestedTable() && this.table != null) {
            this.lastBreak = null;
        } else if (this.lastBreak != null) {
            this.lastBreak.switchControlWord(getRow().getTable()
                    .isNestedTable() ? TABLE_CELL_NESTED_PARAGRAPH
