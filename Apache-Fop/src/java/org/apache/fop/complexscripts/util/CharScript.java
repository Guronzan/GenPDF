...
public final class CharScript {
    ...
    public static int dominantScript(final CharSequence cs) {
        ...
        for (int i = 0, n = cs.length(); i < n; i++) {
            ...
            final Integer v = (Integer) m.get(k);
            if (v != null) {
        ...
        for (final Iterator it = m.entrySet().iterator(); it.hasNext();) {
            ...
            final Integer k = (Integer) e.getKey();
            ...
            case SCRIPT_UNCODED:
                break;
            default: {
                final Integer v = (Integer) e.getValue();
                assert v != null;
    ...
    private static void putScriptTag(final Map tm, final Map cm,
            final int code, final String tag) {
        assert tag != null;
        assert tag.length() != 0;
        assert code >= 0;
        assert code < 2000;
    ...
    private static void makeScriptMaps() {
        final HashMap<Integer, String> tm = new HashMap<Integer, String>();
        final HashMap<String, Integer> cm = new HashMap<String, Integer>();
