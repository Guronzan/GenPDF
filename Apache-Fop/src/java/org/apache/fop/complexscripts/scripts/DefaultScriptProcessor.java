...
/**
 * <p>
 * Default script processor, which enables default glyph
 * composition/decomposition, common ligatures, localized forms and kerning.
 * </p>
 *
 * <p>
...
public class DefaultScriptProcessor extends ScriptProcessor {
    ...
    DefaultScriptProcessor(final String script) {
        super(script);
    }
    ...
    public GlyphSequence reorderCombiningMarks(final GlyphDefinitionTable gdef,
        ...
        // only reorder if there is at least one mark and at least one non-mark
        // glyph
        if (nm > 0 && ng - nm > 0) {
            final GlyphSequence.CharAssociation[] aa = gs
                    .getAssociations(0, -1);
            final int[] nga = new int[ng];
            final int[][] npa = gpa != null ? new int[ng][] : null;
            ...
            for (int i = 0; i < ng; i++) {
                final int gid = ga[i];
                final int[] pa = gpa != null ? gpa[i] : null;
                final GlyphSequence.CharAssociation ca = aa[i];
                if (gdef.isGlyphClass(gid,
