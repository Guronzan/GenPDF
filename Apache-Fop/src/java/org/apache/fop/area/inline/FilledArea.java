...
public class FilledArea extends InlineParent {
     ...
     * 
     * private void setChildOffset(ListIterator childrenIterator, int v) { while
     * (childrenIterator.hasNext()) { InlineArea child = (InlineArea)
     * childrenIterator.next(); if (child instanceof InlineParent) {
     * setChildOffset(((InlineParent) child).getChildAreas().listIterator(), v);
     * } else if (child instanceof InlineViewport) { // nothing } else {
    ...
    public List<InlineArea> getChildAreas() {
        ...
        final List<InlineArea> newList = new ArrayList<InlineArea>();
