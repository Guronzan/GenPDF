...
public final class FileCacheSeekableStream extends SeekableStream {
    ...
    /** The read buffer. */
    private final byte[] buf = new byte[this.bufLen];
    ...
    private long readUntil(final long pos) throws IOException {
        ...
        while (len > 0) {
            ...
            final int nbytes = this.stream.read(this.buf, 0,
                    (int) Math.min(len, this.bufLen));
            if (nbytes == -1) {
                this.foundEOF = true;
                return this.length;
            }

            this.cache.setLength(this.cache.length() + nbytes);
    ...
    public void seek(final long pos) throws IOException {
        if (pos < 0) {
            throw new IOException(
                    PropertyUtil.getString("FileCacheSeekableStream0"));
        }
        this.pointer = pos;
    }
    ...
    public int read(final byte[] b, final int off, int len) throws IOException {
        ...
        len = (int) Math.min(len, pos - this.pointer);
        if (len > 0) {
            this.cache.seek(this.pointer);
