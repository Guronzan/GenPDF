...
/**
 * Helper methods commonly used when dealing with DSC-compliant PostScript
 * files.
 */
public final class DSCTools implements DSCParserConstants {
    ...
    /**
     * Indicates whether the given event ends a header comment section according
     * to the rules in DSC 3.0, chapter 4.4.
     * 
     * @param event
     *            the event to check
     * @return true if a header comment section would be ended either
     *         explicitely or implicitely by the given event
     */
    public static boolean headerCommentsEndHere(final DSCEvent event) {
        switch (event.getEventType()) {
        case DSC_COMMENT:
            final DSCComment comment = event.asDSCComment();
            return comment.getName().equals(DSCConstants.END_COMMENTS);
        case COMMENT:
            final String s = ((PostScriptComment) event).getComment();
     ...
     */
    public static DSCHeaderComment checkAndSkipDSC30Header(
            final DSCParser parser) throws DSCException, IOException {
        if (!parser.hasNext()) {
            throw new DSCException("File has no content");
        }
        final DSCEvent event = parser.nextEvent();
        if (event.getEventType() == HEADER_COMMENT) {
            final DSCHeaderComment header = (DSCHeaderComment) event;
    ...
    /**
     * Advances the parser to the next page or to the trailer or the end of file
     * comment.
     * 
     * @param parser
     *            the DSC parser
     * @param gen
     *            the PSGenerator instance to pass the skipped events through to
     ...
     *             In case of a violation of the DSC spec
     */
    public static DSCComment nextPageOrTrailer(final DSCParser parser,
