...
public final class Glyphs {
     ...
     * 
     * @deprecated That array was supposed to represent the standard Macintosh
     *             ordering of glyphs in a TrueType font (it does NOT correspond
     *             to the MacRoman encoding). In addition some entries are
     *             incorrect.
     */
    @Deprecated
    ...
    static {
        ...
        addAlternatives(map, new String[] { "hyphen", "sfthyphen",
                "softhyphen", "minus" });
        ...
        addAlternatives(map, new String[] { "periodcentered", "middot",
                "bulletoperator", "anoteleia" });
        // space maps to 0020 (SPACE) and 00A0 (NO-BREAK SPACE)
        addAlternatives(map, new String[] { "space", "nonbreakingspace",
                "nbspace" });
    ...
    private static void addAlternatives(final Map map,
    ...
    private static String[] loadGlyphList(final String filename,
            final Map charNameToUnicodeMap) {
        final List lines = new java.util.ArrayList();
        final InputStream in = Glyphs.class.getResourceAsStream(filename);
        if (in == null) {
    ...
    public static String getUnicodeSequenceForGlyphName(String glyphName) {
        ...
        while (tokenizer.hasMoreTokens()) {
            final String token = tokenizer.nextToken();
            // Step 3
            final String sequence = (String) CHARNAMES_TO_UNICODE.get(token);
            if (sequence == null) {
     ...
     * @deprecated User getUnicodeCodePointsForGlyphName instead. This method
     *             only returns the first Unicode code point it finds.
     */
    @Deprecated
