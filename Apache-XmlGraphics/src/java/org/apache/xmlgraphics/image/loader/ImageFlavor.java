...
/**
 * The flavor of an image indicates in which form it is available. A bitmap
 * image loaded into memory might be represented as a BufferedImage (indicated
 * by ImageFlavor.BUFFERED_IMAGE). It is mostly used by consuming code to
 * indicate what kind of flavors can be processed so a processing pipeline can
 * do the necessary loading operations and conversions.
 */
public class ImageFlavor {

    /** An image in form of a RenderedImage instance */
    public static final ImageFlavor RENDERED_IMAGE = new ImageFlavor(
            "RenderedImage");
    ...
    public static final ImageFlavor XML_DOM = new MimeEnabledImageFlavor(DOM,
            "text/xml");
    ...
    public static final ImageFlavor RAW_CCITTFAX = new ImageFlavor(
            "RawCCITTFax");
    /**
     * An image in form of a Graphics2DImage (can be painted on a Graphics2D
     * interface)
     */
    public static final ImageFlavor GRAPHICS2D = new ImageFlavor(
            "Graphics2DImage");

    private final String name;

    /**
     * Constructs a new ImageFlavor. Please reuse existing constants wherever
     * possible!
     * 
     * @param name
    ...
    /**
     * Returns the MIME type that the image flavor represents if a MIME type is
     * available. This is only applicable to images which can also exist as
     * files. For images flavors like decoded in-memory images
     * (Rendered/BufferedImage), this method will return null.
     * 
     * @return the MIME type or null if no MIME type can be provided (like for
     *         in-memory images)
     */
    ...
    /**
     * Returns the XML namespace URI that the image flavor represents if such a
     * namespace URI is available. This is only applicable to images in XML
     * form. Other image types will return null.
     * 
     * @return the XML or null if no MIME type can be provided (like for
     *         in-memory images)
     */
