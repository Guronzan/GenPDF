...
public class TIFFImageEncoder extends ImageEncoderImpl {
    ...
    public void encode(final RenderedImage im) throws IOException {
        ...
        final Iterator iter = encodeParam.getExtraImages();
        if (iter != null) {
            int ifdOffset = 8;
    ...
    private int encode(final RenderedImage im,
        ...
        // Create Directory
        final SortedSet fields = new TreeSet();
        ...
        if (extraFields != null) {
            final List extantTags = new ArrayList(fields.size());
            final Iterator fieldIter = fields.iterator();
            while (fieldIter.hasNext()) {
                final TIFFField fld = (TIFFField) fieldIter.next();
                extantTags.add(new Integer(fld.getTag()));
            }

            final int numExtraFields = extraFields.length;
            for (int i = 0; i < numExtraFields; i++) {
                final TIFFField fld = extraFields[i];
                final Integer tagValue = new Integer(fld.getTag());
        ...
        final boolean jpegRGBToYCbCr = false;
    ...
    private int getDirectorySize(final SortedSet fields) {
        ...
        while (iter.hasNext()) {
            // Get the field.
            final TIFFField field = (TIFFField) iter.next();
    ...
    /**
     * Returns the current offset in the supplied OutputStream. This method
     * should only be used if compressing data.
     */
     ...
     */
    private static int packBits(final byte[] input, int inOffset,
            final int inCount, final byte[] output, int outOffset) {
